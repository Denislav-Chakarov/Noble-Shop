...
function showSearch() {
  ...
  searchBtn.addEventListener('click', () => {
    ...
    document.getElementsByClassName('triangle')[0].classList.toggle('triangle-active');
  });
}
showSearch();
...
function showFilterList() {
  ...
  filterBtn.addEventListener('click', () => {
    ...
    document.getElementsByClassName('triangle2')[0].classList.toggle('triangle2-active');
...
function animateSort() {
  const list2 = document.querySelector('.ProductsRol').animate([
    { transform: 'scale(0)' },
    { transform: 'scale(1)' }
  ], {
    duration: 500,
...
function MenSort() {
  ...
  newArr = [];
  for (var j = 0; j < list_items.length; j++) {
    if (list_items[j].categorie === "Men" || list_items[j].categorie === "Belt") {
      newArr.push(list_items[j]);
    }
  }
}
...
function displayItems(arr = []) {
  ...
  const productHTML = arr.map(product => {
    ...
    return `<div class="col-12 col-lg-3 Product">
         <div class="ProductTop">
           <img class="ProductImage" width="270" height="334" src="`+ img + `">
...
function viewProdct() {
  ...
  viewBtns.forEach(product => {
    product.addEventListener('click', () => {
      ...
      document.getElementsByClassName('darkness')[0].classList.toggle('darkness-active');
...
function LocalStoragePush() {
  ...
  addToCart.addEventListener('click', (e) => {
    ...
    let productPrice = button.getElementsByClassName('ProductPrice')[0].innerText;
    ...
    let productQuantity = button.getElementsByClassName('ProQuantity')[0].innerText;
    productPrice = parseFloat(productPrice);
    productQuantity = parseInt(productQuantity);

    const newProduct = { name: productName, price: productPrice, size: productSize, color: productColor, inCart: productQuantity };
    ...
    return `<div class="ProductCol">
         ...
         <span><i class="fas fa-times DeleteProduct"></i>&ensp;</span>
             ...
             <div class="CartProduct">
                 ...
                 <br>
                 <br>
                    <span class="InCart">`+ inCart + `</span><span>&ensp;x</span><span>&ensp;$` + price + `</span>
                    <p><span>`+ size + `</span><span>&ensp;` + color + `</span></p>
                 </div>
             </div>
         </div>`;
  });
...
function totalCash() {
  ...
  productPrice = parseFloat(productPrice);
  document.querySelector('.SpanPrice').innerText = "$" + Math.round(productPrice * 100) / 100;
}
